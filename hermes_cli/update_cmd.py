...
def _reconcile_diverged_checkout(git_cmd, branch: str, pre_pull_sha, *, target_ref=None) -> None:
    ...
    local_ahead = _git_run(git_cmd, ["rev-list", f"{merge_ref}..HEAD", "--count"])
    try:
        ...
        print(
            ...
            f"origin/{branch}.")
        print(f"  Inspect `git status` and `git log origin/{branch}..HEAD` before retrying.")
        sys.exit(1)
    ...
    # (or orphan divergence: no common ancestor, corrupted HEAD, re-init). Park pre_pull_sha
    # behind a rescue ref before the reset regardless — the reflog is the only other way back.
