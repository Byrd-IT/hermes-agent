...
| `kanban_attach` | Attach a small file inline (base64). Stored as a real attachment under the task's attachments dir, capped at 25 MB; unsafe above about 1–2 KB because model emission can silently corrupt bytes. | `HERMES_KANBAN_TASK` or `kanban` toolset |
| `kanban_attach_file` | Copy a scoped local source path server-side into a task attachment, avoiding model-transcribed bytes (25 MB cap). | `HERMES_KANBAN_TASK` or `kanban` toolset |
