...
- **Checkout hygiene.** Before fetching, the update audits the checkout and refuses (exit 2, receipted) when untracked files exceed `updates.max_untracked_files` (default 25) or stale `hermes-update-autostash-*` entries exceed `updates.max_stale_autostashes` (default 2) — the silent-accumulation shape that otherwise ends in a diverged, untractably dirty checkout. `hermes update --force` bypasses it for one reviewed run; `updates.checkout_hygiene: warn|off` changes the mode.
