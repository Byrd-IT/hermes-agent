...
KANBAN_GUIDANCE = (
    ...
    "`<project-slug>/<task-id>` — the main repo is two levels up, so run `git worktree add` from there. A `scratch` "
    "workspace is an empty, non-git, ephemeral dir: never assume a repo in it — use `git -C <repo>` on the named repo, "
    "or clone/fetch the PR into scratch for review.\n"
    ...
    "- **Attachments.** Attach real downloadable artifacts instead of pasting links in comments: `kanban_attach_file` "
    "(server-side scoped local path; preferred), `kanban_attach` (base64 only for small payloads; unsafe above ~1–2 KB), "
    "or `kanban_attach_url` (server-side public http(s) fetch); `kanban_complete(artifacts=[...])` also stages workspace files. "
    "All have a 25 MB cap; `kanban_attachments` lists them. "
