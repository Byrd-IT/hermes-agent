...
@pytest.mark.parametrize('history,failure,keep', [
    ...
    ('orphan', None, False), ('orphan', 'head', False),
])
def test_update_refuses_reset_and_preserves_local_work(
    update_tree, monkeypatch, capsys, history, failure, keep,
):
    """Byrd-IT guard (e82bf770cc) on real Git: upstream resets a diverged ``main`` after
    parking a rescue ref; this deployment carries fleet fixes on ``main``, so a same-branch
    divergence with commits outside ``origin/main`` (ordinary or orphan) must stop before any
    ``reset --hard``, leave HEAD and refs alone, and keep the autostashed work recoverable."""
    t = update_tree
    ...
    assert not git(t.clone, 'for-each-ref', 'refs/hermes-update-backups/')
    assert git(t.clone, 'show', 'stash@{0}^3:untracked.txt') == 'local edit'
    output = capsys.readouterr().out
    assert 'Refusing to reset main: 1 local commit(s)' in output
